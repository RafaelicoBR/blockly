...
turtlepage.polygon = function(opt_data, opt_ignored, opt_ijData) {
  return '<block type="procedures_defnoreturn" ' + opt_data.modifiers + '><mutation>' + ((opt_data.length == 0) ? '<arg name="length"></arg>' : '') + '</mutation><title name="NAME">' + opt_data.title + '</title><statement name="STACK"><block type="controls_repeat_ext" editable="false" deletable="false" movable="false"><value name="TIMES"><block type="math_number" editable="false" deletable="false" movable="false"><title name="NUM">' + opt_data.sides + '</title></block></value><statement name="DO"><block type="draw_move" editable="false" deletable="false" movable="false"><value name="VALUE">' + ((opt_data.length == 0) ? '<block type="variables_get_length"></block>' : '<block type="math_number" editable="false" deletable="false" movable="false"><title name="NUM">' + opt_data.length + '</title></block>') + '</value><next><block type="draw_turn" editable="false" deletable="false" movable="false"><value name="VALUE"><block type="math_number" editable="false" deletable="false" movable="false"><title name="NUM">' + 360 / opt_data.sides + '</title></block></value></block></next></block></statement></block></statement></block>';
};
...
turtlepage.start = function(opt_data, opt_ignored, opt_ijData) {
  ...
  if (opt_ijData.page == 1) {
    ...
    if (opt_ijData.level == 3 || opt_ijData.level == 5) {
      ...
      for (var i165 = 25; i165 < 61; i165 += 5) {
        output += '<block type="draw_move" ' + ((i165 == 25) ? 'x="300" y="100"' : '') + ' inline="false" editable="false" disabled="true"><title name="DIR">moveForward</title><value name="VALUE"><block type="math_number"><title name="NUM">' + soy.$$escapeHtml(i165) + '</title></block></value><next><block type="draw_turn" inline="false" editable="false" disabled="true"><title name="DIR">turnRight</title><value name="VALUE"><block type="math_number"><title name="NUM">90</title></block></value><next>';
      }
      for (var i173 = 5; i173 < 25; i173 += 5) {
  ...
  if (opt_ijData.reinf) {
    ...
    switch (opt_ijData.reinf[1]) {
      ...
      case 'picture-table':
        output += '<table>';
        var tupleList270 = opt_ijData.reinf[2];
        var tupleListLen270 = tupleList270.length;
        for (var tupleIndex270 = 0; tupleIndex270 < tupleListLen270; tupleIndex270++) {
          var tupleData270 = tupleList270[tupleIndex270];
          output += '<tr height="100" valign="middle"><td><img src="' + soy.$$escapeHtml(tupleData270[1]) + '"></td><td>' + soy.$$escapeHtml(tupleData270[0]) + '</td></tr>';
        }
        output += '</table>';
        break;
      case 'quiz':
        ...
        var iLimit286 = opt_ijData.reinf[2][0] + 1;
        for (var i286 = 1; i286 < iLimit286; i286++) {
          output += '<td><img src="p' + soy.$$escapeHtml(opt_ijData.page) + '-l' + soy.$$escapeHtml(opt_ijData.level) + '-a' + soy.$$escapeHtml(i286) + '.png" class="answer" onclick="alert(' + ((i286 == opt_ijData.reinf[2][1]) ? '\'You got it!\'); document.getElementById(\'continueButton\').style.display = \'inline\';' : '\'Wrong!  Try again.\');') + '"></td>';
        }
        output += '</tr></table>';
        break;
      case 'animations':
        ...
        var fileList312 = opt_ijData.reinf[2];
        var fileListLen312 = fileList312.length;
        for (var fileIndex312 = 0; fileIndex312 < fileListLen312; fileIndex312++) {
          var fileData312 = fileList312[fileIndex312];
          output += '<td><img src="' + soy.$$escapeHtml(fileData312) + '"></td>';
        }
        output += '</tr></table></div>';
        break;
      case 'special':
    ...
    var iLimit334 = opt_ijData.maxLevel + 1;
    for (var i334 = 1; i334 < iLimit334; i334++) {
      output += (i334 == opt_ijData.level) ? '<span class="tab" id="selected">' + soy.$$escapeHtml(i334) + '</span>' : '<a class="tab" href="?page=' + soy.$$escapeHtml(opt_ijData.page) + '&level=' + soy.$$escapeHtml(i334) + '">' + soy.$$escapeHtml(i334) + '</a>';
    }
      ...
      case 2:
        switch (opt_ijData.level) {
          ...
          case 8:
            ...
            break;
          case 9:
            output += 'Draw a family of snowmen with heights of 110, 100, 90, 80, and 70, each 50 dots apart.';
            break;
          case 10:
